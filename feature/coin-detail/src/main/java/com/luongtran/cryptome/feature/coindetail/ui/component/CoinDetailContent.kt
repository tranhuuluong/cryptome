...
fun CoinDetailContent(
    ...
    val (priceChange24h, priceChange24hFormatted) = uiModel.priceChange24h
    ...
    Column(
        ...
        Row(
            ...
            Text(
                modifier = Modifier.alignBy(LastBaseline),
                text = priceChange24hFormatted,
                style = MaterialTheme.typography.bodyLarge,
                color = if (priceChange24h.toDouble() >= 0) AccentGreen else AccentRed
            )
        }
        ...
        Box(
            ...
            val priceHistory = uiModel.priceHistory
            when {
                priceHistory.size < 2 -> Text(
                    text = "Not enough data to show price history",
                    style = MaterialTheme.typography.bodyMedium,
                    color = MaterialTheme.colorScheme.onSurfaceVariant
                )

                else -> PriceHistoryLineChart(
